...
export async function generateCmdCommitMessage(cwd: string): Promise<string> {
  ...
  throw new Error(
    snippet
      ...
      : "Could not generate a commit message. CommandCode returned no text.",
  );
}

export async function generateCmdPrContent(
  cwd: string,
...
export async function generateCmdBranchName(
  cwd: string,
  message: string,
): Promise<string | null> {
  try {
