...
export async function generateAgyCommitMessage(cwd: string): Promise<string> {
  ...
  throw new Error(
    snippet
      ...
      : "Could not generate a commit message. Antigravity returned no text.",
  );
}

export async function generateAgyPrContent(
  cwd: string,
...
export async function generateAgyBranchName(
  cwd: string,
  message: string,
): Promise<string | null> {
  try {
